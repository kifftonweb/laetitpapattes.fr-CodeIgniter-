...
defined ('BASEPATH') OR exit('No direct script access allowed');
 ...
 class Authentification_user extends CI_Model {
      ...
      protected $_id; 
       ...
       public function __get($key) {
        ...
        protected function clear_session(){
           $this->session->authentification_user = NULL;
        }
        protected function get_property_id(){
         ...
         protected function load_from_session() {
             if ($this->session->authentification_user) {
                $this->_id = $this->session->authentification_user['id']; 
                $this->_username = $this->session->authentification_user['username'];
             }else{
                $this->clear_data();
             }
         }
         ...
         protected function save_session() {
             $this->session->authentification_user = [
                 'id' => $this->_id,
